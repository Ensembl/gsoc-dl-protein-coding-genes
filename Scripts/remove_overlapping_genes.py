...
def filter_sequences(output_dir, gff_file, fasta_file):
    ...
    out_gff_file = os.path.join(output_dir, get_filename_from_path(gff_file) + '_filtered_removed_sequences_overlapping_genes.gff')
    out_fasta_file = os.path.join(output_dir, get_filename_from_path(fasta_file) + '_filtered_removed_sequences_overlapping_genes.fasta')
    ...
    with open(out_fasta_file, 'w') as outfile:
        for record in SeqIO.parse(fasta_file, "fasta"):
            ...
            last_end = 0
            new_sequence = ""
            for id, seq_id, (start, end) in sorted(overlapping_gene_ids_ranges_extended, key=lambda x: x[1][0]):
                if seq_id == record.id:
...
def main():
    parser = argparse.ArgumentParser(description='Filter sequences and genes to exclude overlapping genes.')

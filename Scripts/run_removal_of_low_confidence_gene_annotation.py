...
# Argument parser
parser = argparse.ArgumentParser(description='Creates bsub jobs to filter genes based on quality.')
...
# Get all GFF files
gff_files = sorted([f for f in os.listdir(args.input_dir) if f.endswith('_annotations.gff')])
...
"""
    ...
    fasta_file = gff_file.replace('_annotations.gff', '_sequences.fasta')

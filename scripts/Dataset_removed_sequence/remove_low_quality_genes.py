...
def filter_sequences(gff_file, fasta_file):
    # Derive the output filenames from the input filenames
    out_gff_file = os.path.splitext(gff_file)[0] + '_filtered_removed_sequences.gff'
    out_fasta_file = os.path.splitext(fasta_file)[0] + '_filtered_removed_sequences.fasta'
...
def main():
    ...
    filter_sequences(args.gff_file, args.fasta_file)

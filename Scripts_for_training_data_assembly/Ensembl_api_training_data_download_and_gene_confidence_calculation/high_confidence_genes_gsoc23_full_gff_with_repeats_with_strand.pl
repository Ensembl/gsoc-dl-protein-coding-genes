...
foreach my $slice (@$slices) {
  ...
  # Get all repeat features in the slice
  my $repeats = $slice->get_all_RepeatFeatures();
  
  foreach my $repeat (@$repeats) {
    # Get the repeat information
    my $repeat_id = "repeat_" . $repeat_number;
    $repeat_number++;
    my $repeat_start = $repeat->start();
    my $repeat_end = $repeat->end();
    my $repeat_type = $repeat->repeat_consensus()->repeat_type();
    my $repeat_class = $repeat->repeat_consensus()->repeat_class();

    # Write repeat information to the GFF file
    say $gff_out join("\t", $slice->seq_region_name(), 'Ensembl', $repeat_type, $repeat_start, $repeat_end, '.', '.', '.', "ID=$repeat_id;class=$repeat_class"), "\n";
  }
  ...
  foreach my $gene (@$genes) {
    ...
    my $transcript_strand = $transcript->strand() == 1 ? '+' : '-';
    my $ise = $transcript->get_all_IntronSupportingEvidence();
    my $introns = $transcript->get_all_Introns();

    my $five_prime_utr_sequence = $transcript->five_prime_utr();
    my $three_prime_utr_sequence = $transcript->three_prime_utr();
    my $utr_strand = $transcript_strand;

    print "Five prime UTR object type: " . ref($five_prime_utr_sequence) . "\n";
    print "Three prime UTR object type: " . ref($three_prime_utr_sequence) . "\n";
    ...
    my $transcript_sequence = $transcript->seq();
    ...
    foreach my $exon (@$exons) {
        ...
        my $exon_strand = $exon->strand() == 1 ? '+' : '-';
        say $gff_out join("\t", $slice->seq_region_name(), 'Ensembl', 'exon', $exon_start, $exon_end, '.', $exon_strand, '.', "Parent=$transcript_id;ID=$exon_id"), "\n";

    }
    
    # Write UTR information to the GFF file
    if (defined $five_prime_utr_sequence) {
        my $utr_id = $transcript_id . '_5UTR';
        my $utr_start = index(uc($transcript_sequence), uc($five_prime_utr_sequence)) + 1;
        my $utr_end = $utr_start + length($five_prime_utr_sequence) - 1;
        say $gff_out join("\t", $slice->seq_region_name(), 'Ensembl', 'five_prime_UTR', $utr_start, $utr_end, '.', $utr_strand, '.', "Parent=$transcript_id;ID=$utr_id"), "\n";
    }

    if (defined $three_prime_utr_sequence) {
        my $utr_id = $transcript_id . '_3UTR';
        my $utr_start = index(uc($transcript_sequence), uc($three_prime_utr_sequence)) + 1;
        my $utr_end = $utr_start + length($three_prime_utr_sequence) - 1;
        say $gff_out join("\t", $slice->seq_region_name(), 'Ensembl', 'three_prime_UTR', $utr_start, $utr_end, '.', $utr_strand, '.', "Parent=$transcript_id;ID=$utr_id"), "\n";
    }
    ...
    foreach my $cds (@cds_objects) {
        ...
        say $gff_out join("\t", $slice->seq_region_name(), 'Ensembl', 'mRNA', $transcript_start, $transcript_end, '.', $transcript_strand, '.', "Parent=$gene_id;ID=$transcript_id"), "\n";
    }	
    ...
    foreach my $intron (@$introns) {
        ...
        my $intron_id = $transcript_id . '_intron_' . $intron_counter++;
        my $intron_start = $intron->seq_region_start();
        my $intron_end = $intron->seq_region_end();
        
        # Write intron information to the GFF file
        my $intron_strand = $intron->strand() == 1 ? '+' : '-';
        say $gff_out join("\t", $slice->seq_region_name(), 'Ensembl', 'intron', $intron_start, $intron_end, '.', $intron_strand, '.', "Parent=$transcript_id;ID=$intron_id"), "\n";

...
class SimpleClassifier(nn.Module):
    ...
    def loss(self, x, tags):  
        ...
        loss = F.cross_entropy(adjusted_features.view(-1, adjusted_features.shape[-1]), tags.view(-1), ignore_index=-1)  # ignore padding index
...
def get_model_predictions_and_labels(model, dataloader):
    ...
    with torch.no_grad():
        for inputs, labels, _ in dataloader:  # No mask tensor
            inputs, labels = inputs.to(device), labels.to(device)
            outputs = model(inputs)  
            _, predicted_labels = torch.max(outputs, -1)
            y_true.append(labels.view(-1).tolist())
            y_pred.append(predicted_labels.view(-1).tolist())

    y_true = flatten_list(y_true)
    y_pred = flatten_list(y_pred)

    # Remove padding indices
    y_true, y_pred = zip(*[(true, pred) for true, pred in zip(y_true, y_pred) if true != -1])

    return y_true, y_pred

def train_simple_classifier(train_dataloader, input_dim, num_tags, num_epochs):
    model = SimpleClassifier(input_dim, num_tags).to(device)
    learning_rate = 0.001  
    ...
    for epoch in range(num_epochs):
        total_loss = 0
        for i, (inputs, tags, _) in enumerate(train_dataloader):  # No mask tensor
            inputs, tags = inputs.to(device), tags.to(device)
            optimizer.zero_grad()
            with autocast():
                tags = tags.squeeze(-1)
                loss = model.loss(inputs, tags)  # No mask tensor
